...
| 2026-08-31 | `cargo test --manifest-path src-tauri/Cargo.toml --lib --no-default-features` - 82 passed | Core application state including automatic trust, mixed confirmation settings, discovery-identity mismatch and revocation; plus scheduler, receiver, events, lifecycle, diagnostics, notifications, and native actions. |
