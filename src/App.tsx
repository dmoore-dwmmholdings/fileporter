...
export default function App({ initialSnapshot = emptySnapshot }: FileporterAppProps) {
  ...
  if (!snapshot.onboardingComplete) return <Onboarding onComplete={(next) => { revisionRef.current = next.revision; setSnapshot(toViewModel(next)); setScreen('devices'); setShowPairingStep(true); }} />;
  if (showPairingStep) return <AppFrame snapshot={snapshot} screen="devices" onNavigate={setScreen}><section className="tab-panel" aria-labelledby="pair-onboarding-heading"><div className="panel-heading"><h1 id="pair-onboarding-heading">Pair another device</h1><p>Nearby devices can appear after your operating system allows local-network access. Compare the security code on both devices before confirming.</p></div><DevicesView snapshot={snapshot} /><button className="inline-action" type="button" onClick={() => { setShowPairingStep(false); setScreen('home'); }}>Skip for now</button></section></AppFrame>;
  ...
  return (
    ...
    >
      ...
      {stagedPaths.length > 0 && <section className="empty-targets" aria-labelledby="staged-send-heading"><strong id="staged-send-heading">{stagedPaths.length} item{stagedPaths.length === 1 ? '' : 's'} ready to send</strong><span>Choose a trusted device. Items will send automatically when it is available.</span>{offlineTrustedDevices.length ? <div className="staged-recipients" aria-label="Trusted offline recipients">{offlineTrustedDevices.map((device) => <button key={device.id} type="button" className={stagedTargetIds.includes(device.id) ? 'recipient active' : 'recipient'} aria-pressed={stagedTargetIds.includes(device.id)} onClick={() => toggleStagedTarget(device.id)}>{device.name}</button>)}</div> : <span className="muted">Pair a device to send while it is offline.</span>}<div><button type="button" onClick={() => setScreen('devices')}>Pair a device</button><button type="button" disabled={!stagedTargetIds.length} onClick={() => { void sendStaged(); }}>Send when available</button><button type="button" className="text-button" onClick={() => { setStagedPaths([]); setStagedTargetIds([]); }}>Clear</button></div></section>}
      {selectedDeviceIds.length === 0 && stagedPaths.length === 0 && <section className="empty-targets" aria-live="polite"><strong>No devices online</strong><span>Select files or folders to choose a trusted device and send when it is available.</span></section>}
...
function Onboarding({ onComplete }: OnboardingProps) {
  ...
  async function complete() {
    ...
    try {
      const next = await appBridge.completeOnboarding({ deviceName: deviceName.trim(), receiveDirectory: directory, launchAtLogin, notificationsEnabled });
      onComplete(next);
    } catch { setError('Fileporter could not save setup. Your folder has not been enabled for receiving.'); }
    finally { setSaving(false); }
  }
  return <main className="onboarding-shell"><section className="onboarding-card"><div className="brand"><span className="brand-mark"><MonitorUp size={18} /></span>Fileporter</div><h1>Send files directly to your other computers on this network.</h1><p className="muted">On macOS, Fileporter asks to find devices on your local network. On Windows, allow its firewall prompt on Private networks. You can recover either permission later in system settings.</p><label>Device name<input value={deviceName} maxLength={48} onChange={(event) => setDeviceName(event.target.value)} placeholder="This computer" autoFocus /></label><label>Receive folder<span className="field-row"><input value={directory ?? ''} readOnly placeholder="Choose where received files go" /><button type="button" onClick={() => { void chooseDirectory(); }}><FolderOpen size={16} /> Choose</button></span></label><label className="check-row"><input type="checkbox" checked={launchAtLogin} onChange={(event) => setLaunchAtLogin(event.target.checked)} /> Launch Fileporter when I sign in</label><label className="check-row"><input type="checkbox" checked={notificationsEnabled} onChange={(event) => setNotificationsEnabled(event.target.checked)} /> Notify me when files arrive</label>{error && <p className="form-error" role="alert">{error}</p>}<button className="primary-action" type="button" disabled={!validName || !directory || saving} onClick={() => { void complete(); }}>{saving ? 'Saving…' : 'Continue to pairing'}</button></section></main>;
}
...
function SettingsView({ snapshot, onBack, onSnapshot }: SettingsViewProps) {
  ...
  const restore = () => { setDeviceName(snapshot.localDeviceName); setReceiveDirectory(snapshot.receiveDirectory ?? ''); setReceivingEnabled(snapshot.lifecycle.receivingEnabled); setListenAddress(snapshot.settings.preferredListenAddress); setLaunchAtLogin(snapshot.launchAtLogin); setNotificationsEnabled(snapshot.notificationsEnabled); setHistoryRetentionDays(snapshot.settings.historyRetentionDays as 0 | 7 | 30 | 90); setError(null); setStatus(null); };
